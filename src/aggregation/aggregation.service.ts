...
export class AggregationService {
  ...
  async aggregateDataByUserId(userId: string) {
    ...
    const balance = totals[TransactionType.EARNED] - totals[TransactionType.SPENT] - totals[TransactionType.PAYOUT];
  ...
  async getAggregatedPayouts() {
    const payouts = await this.transactionRepository.find({ where: { type: TransactionType.PAYOUT } });
